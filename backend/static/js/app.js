...
if (false && detectLocationBtn && euroPlaceInput) {
...
function renderChatHistory(history = []) {
  if (!chatMessages || !history.length) return;

  chatMessages.innerHTML = "";
  history.forEach((item) => {
    if (item.query) appendChatMessage(item.query, "user");
    if (item.answer) appendChatMessage(item.answer, "system");
  });
}
...
async function loadAssistantHistory() {
  ...
  try {
    const response = await fetch("/ai/api/history/");
...
if (authModal && loginBtn && signupBtn && authClose && authSubmit && toggleMode && repeatField && legalText) {
