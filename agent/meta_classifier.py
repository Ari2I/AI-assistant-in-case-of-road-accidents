"""
Мета-классификатор v2 — исправлены границы категорий,
добавлен keyword-override до LLM-вызова.

Исправления v2.1:
  - _KEYWORD_OVERRIDES: europrotocol_impossible теперь проверяется РАНЬШЕ
    insurance_communication, чтобы "нет страховки у пострадавшего" не уходило
    в страховую категорию.
"""

from __future__ import annotations

import json
import re

from gigachat import GigaChat

_CATEGORIES = {
    "general_questions": (
        "Общие вопросы по ДТП, ПДД, ОСАГО, не связанные с конкретным этапом оформления. "
        "Примеры: 'какая дистанция для знака', 'штраф за отсутствие ОСАГО', 'можно ли буксировать', "
        "'сколько времени на оформление', 'лимиты выплат', 'что такое европротокол'. "
        "НЕ входит: пользователь сообщает о текущем ДТП и просит пошаговую инструкцию — это first_steps. "
        "НЕ входит: любые вопросы НЕ про ДТП/ОСАГО/ПДД — это оффтопик."
    ),
    "first_steps": (
        "Пользователь только что попал в ДТП, ещё на месте, не знает с чего начать. "
        "Примеры: 'попал в ДТП', 'что делать', 'авария произошла'. "
        "НЕ входит: вопросы о документах, страховой, заполнении бланков. "
        "ВАЖНО: если в запросе нет упоминания ДТП/аварии — это не эта категория."
    ),
    "europrotocol_possible": (
        "Вопросы о том, КАК оформить ДТП без ГИБДД: условия, приложения, фотофиксация, лимиты. "
        "Примеры: 'можно ли без ГИБДД', 'какое приложение скачать', 'как сфотографировать'. "
        "НЕ входит: заполнение конкретных пунктов бланка, отправка в страховую. "
        "ВАЖНО: только вопросы про Европротокол при ДТП."
    ),
    "europrotocol_impossible": (
        "ДТП, при котором нужна полиция: пострадавшие, 3+ участника, нет ОСАГО, скрылся водитель. "
        "Примеры: 'есть раненые', 'второй уехал', 'нет страховки'. "
        "ВАЖНО: только ситуации ДТП, где Европротокол невозможен."
    ),
    "filling_europrotocol": (
        "Заполнение бланка извещения НА МЕСТЕ ДТП: конкретные пункты 1-18, схема, повреждения, ошибки. "
        "Примеры: 'что писать в пункте 10', 'как нарисовать схему', 'допустил ошибку в бланке'. "
        "НЕ входит: отправка документов в страховую — это insurance_communication. "
        "ВАЖНО: только заполнение протокола после ДТП."
    ),
    "insurance_communication": (
        "Взаимодействие со страховой ПОСЛЕ оформления ДТП: какие документы подать, куда отправить, "
        "сроки, выплаты, споры, прямое возмещение, проверка полиса, КБМ. "
        "Примеры: 'какие документы отправить в страховую', 'куда подавать заявление', "
        "'когда придёт выплата', 'страховая отказала'. "
        "ВАЖНО: любой вопрос про отправку/подачу документов после ДТП — ЭТА категория. "
        "НЕ входит: общие вопросы про страхование, не связанные с ДТП."
    ),
}

# Жёсткие override-правила: если слово найдено в запросе — LLM не вызывается.
# ПОРЯДОК ВАЖЕН: более специфичные правила — выше, менее специфичные — ниже.
_KEYWORD_OVERRIDES: list[tuple[list[str], str, int]] = [
    # 1. Общие вопросы (специфичные составные фразы — не путать с контекстными)
    (
        [
            "дистанция для знака", "расстояние знак дтп", "знак аварийной остановки",
            "штраф за осаго", "штраф без страховки", "буксировка дтп",
            "лимит выплат", "сколько времени на оформление", "срок оформления",
        ],
        "general_questions",
        -1,
    ),
    # 2. Невозможность Европротокола — ВЫШЕ страховой, чтобы "нет страховки" не уходило туда
    (
        [
            "пострадавш", "ранен", "сбил пешехода",
            "скрылся", "уехал с места",
            "три машин", "нет осаго", "нет страховки",
        ],
        "europrotocol_impossible",
        0,
    ),
    # 3. Страховая — после невозможности Европротокола
    (
        [
            "страховую", "страховой", "страховая", "страховщик",
            "заявление о", "выплат", "возмещен",
            "подать документы", "отправить документы", "какие документы",
        ],
        "insurance_communication",
        9,
    ),
]

_PROMPT = """\
Определи категорию вопроса пользователя в контексте ДТП.

История диалога:
{history}

Сообщение пользователя: "{query}"

Категории (читай примеры и границы):
{categories}

Номер блока алгоритма (0-9):
0=безопасность, 1=аварийка/знак, 2=пострадавшие, 3=участники,
4=ОСАГО, 5=разногласия, 6=фиксация, 7=заполнение бланка на месте,
8=фотофиксация, 9=действия после оформления и отправка в страховую

КРИТИЧЕСКИ ВАЖНО:
1. Если запрос НЕ связан с ДТП/ОСАГО/ПДД/ГИБДД — верни relevant: false.
2. Любые вопросы про автомобили без упоминания ДТП — не относятся к нашему функционалу.
3. Личные вопросы, кулинария, погода, развлечения, код, переводы — не относятся к ДТП.
4. Если сомневаешься — используй fallback first_steps, но только если есть связь с ДТП.

Верни ТОЛЬКО валидный JSON:
{{"relevant": true/false, "category": "ключ_категории", "block": число}}
"""


def meta_classify(
    giga: GigaChat,
    query: str,
    history_text: str = "",
) -> dict:
    fallback = {"relevant": True, "category": "first_steps", "block": 0}

    # Keyword override — быстро, без LLM
    query_lower = query.lower()
    for keywords, forced_category, forced_block in _KEYWORD_OVERRIDES:
        if any(kw in query_lower for kw in keywords):
            return {
                "relevant": True,
                "category": forced_category,
                "block": forced_block,
            }

    categories_text = "\n".join(
        f'"{k}": {v}' for k, v in _CATEGORIES.items()
    )
    prompt = _PROMPT.format(
        history=history_text or "(начало диалога)",
        query=query,
        categories=categories_text,
    )

    try:
        resp = giga.chat(prompt)
        text = resp.choices[0].message.content.strip()
        m = re.search(r"\{.*\}", text, re.DOTALL)
        if not m:
            return fallback

        data = json.loads(m.group(0))
        category = data.get("category", "first_steps")
        if category not in _CATEGORIES:
            category = "first_steps"

        block = int(data.get("block", 0))
        if block == -1 or category == "general_questions":
            category = "general_questions"
            block = -1

        return {
            "relevant": bool(data.get("relevant", True)),
            "category": category,
            "block": block,
        }

    except Exception as e:
        print(f"[meta_classifier] error: {e}")
        return fallback